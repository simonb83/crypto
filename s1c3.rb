...
encoded_bytes = hex_to_byte_array(encoded)
...
best = results.sort_by{|i,f| -f}.first(5).map{|key,value| key}

best.each do |key|
	puts "String for KEY=#{key} is #{xor_byte_arrays(encoded_bytes,Array.new(length,key))}"
end
# puts xor_byte_arrays(encoded_bytes,Array.new(length,88))
# puts xor_byte_arrays(encoded_bytes,Array.new(length,29))
